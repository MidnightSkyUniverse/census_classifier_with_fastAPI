...
def go(cfg: DictConfig):
    ...
    for train_x_split, val_x_split in kfold.split(trainval):
        ...
        model = train_RandomForest_model(
            X_train, y_train, cfg.modeling.random_state)
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X, y, test_size=cfg.modeling.test_size, random_state=cfg.modeling.random_state)
    model = train_RandomForest_model(
        X_train, y_train, cfg.modeling.random_state)
    preds = inference(model, X_val)
    precision, recall, fbeta = compute_model_metrics(y_val, preds)
    logger.info(
        f"Full dataset run: precision: {precision}'; recall: {recall}; fbeta: {fbeta}")
